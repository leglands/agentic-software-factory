...
metadata:
  ...
  triggers:
  - when working on fp backend
eval_cases:
- id: fp-backend-approach
  prompt: How should I approach fp backend for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on fp backend
  tags:
  - fp
- id: fp-backend-best-practices
  prompt: What are the key best practices and pitfalls for fp backend?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for fp backend
  tags:
  - fp
  - best-practices
- id: fp-backend-antipatterns
  prompt: What are the most common mistakes to avoid with fp backend?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
