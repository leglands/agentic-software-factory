...
metadata:
  ...
  triggers:
  - 'creating persistent agents with azureaiagentsprovider, using hosted tools (code '
eval_cases:
- id: agent-framework-azure-ai-py-approach
  prompt: How should I approach agent framework azure ai py for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on agent framework azure ai py
  tags:
  - agent
- id: agent-framework-azure-ai-py-best-practices
  prompt: What are the key best practices and pitfalls for agent framework azure ai
    py?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for agent framework azure ai py
  tags:
  - agent
  - best-practices
- id: agent-framework-azure-ai-py-antipatterns
  prompt: What are the most common mistakes to avoid with agent framework azure ai
    py?
  should_trigger: true
  checks:
  - length_min:80
