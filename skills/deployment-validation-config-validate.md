...
eval_cases:
- id: deployment-validation-config-validate-approach
  prompt: How should I approach deployment validation config validate for a production
    system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on deployment validation config validate
  tags:
  - deployment
- id: deployment-validation-config-validate-best-practices
  prompt: What are the key best practices and pitfalls for deployment validation config
    validate?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for deployment validation config validate
  tags:
  - deployment
  - best-practices
- id: deployment-validation-config-validate-antipatterns
  prompt: What are the most common mistakes to avoid with deployment validation config
    validate?
  should_trigger: true
  checks:
  - length_min:80
