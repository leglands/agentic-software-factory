...
metadata:
  ...
  triggers:
  - working with incident response incident response
  - working on incident response incident response tasks or workflows
eval_cases:
- id: incident-response-incident-response-approach
  prompt: How should I approach incident response incident response for a production
    system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on incident response incident response
  tags:
  - incident
- id: incident-response-incident-response-best-practices
  prompt: What are the key best practices and pitfalls for incident response incident
    response?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for incident response incident response
  tags:
  - incident
  - best-practices
- id: incident-response-incident-response-antipatterns
  prompt: What are the most common mistakes to avoid with incident response incident
    response?
  should_trigger: true
  checks:
  - length_min:80
