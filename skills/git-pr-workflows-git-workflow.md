...
metadata:
  ...
  triggers:
  - working on complete git workflow with multi-agent orchestration tasks or workflo
eval_cases:
- id: git-pr-workflows-git-workflow-approach
  prompt: How should I approach git pr workflows git workflow for a production system?
  should_trigger: true
  checks:
  - length_min:150
  - no_placeholder
  expectations:
  - Provides concrete guidance on git pr workflows git workflow
  tags:
  - git
- id: git-pr-workflows-git-workflow-best-practices
  prompt: What are the key best practices and pitfalls for git pr workflows git workflow?
  should_trigger: true
  checks:
  - length_min:100
  - no_placeholder
  expectations:
  - Lists concrete best practices for git pr workflows git workflow
  tags:
  - git
  - best-practices
- id: git-pr-workflows-git-workflow-antipatterns
  prompt: What are the most common mistakes to avoid with git pr workflows git workflow?
  should_trigger: true
  checks:
  - length_min:80
  - no_placeholder
