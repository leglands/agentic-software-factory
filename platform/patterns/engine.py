...
async def _build_node_context(agent: AgentDef, run: PatternRun) -> ExecutionContext:
    ...
    # ── SPECS.md: knowledge bootstrap output (all agents see this) ──
    # Generated by _knowledge_bootstrap() at run start, refreshed after archi phases.
    # This is the "sachant" layer — maintained by documentation agents.
    _specs_md = ""
    if run.project_id:
        try:
            _specs_cached = get_memory_manager().project_retrieve(run.project_id, "specs_md")
            if _specs_cached and _specs_cached.get("value"):
                _specs_md = _specs_cached["value"]
        except Exception:
            pass
    ...
    # Prepend SPECS.md + PROJECT_BRIEF (progressive disclosure — always visible)
    _header = ""
    if _specs_md:
        _header += f"## PROJECT SPECS\n{_specs_md}\n\n"
    elif _project_brief:
        _header += f"## PROJECT BRIEF\n{_project_brief}\n\n"
    if _header:
        project_context = f"{_header}{project_context}" if project_context else _header.rstrip()
